...
@test:Config {}
function testPatientXmlParsing() returns error? {
    string xmlStr = string `<Patient xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="urn:hl7-org:v3" xmlns:sdtc="urn:hl7-org:sdtc" xmlns:voc="urn:hl7-org:v3/voc">
                <name use="L">
                    <given>Eve</given>
    ...
    xml patientXml = check xml:fromString(xmlStr);
    Patient patient = check xmldata:parseAsType(patientXml, {textFieldName: "xmlText"});

    // Assert patient details
    PN[]? names = patient.name;
    if names is PN[] && names.length() > 0 {
        test:assertEquals(names[0].use, "L");
    }

    CE? adminGenderCode = patient.administrativeGenderCode;
    if adminGenderCode is CE {
        test:assertEquals(adminGenderCode.code, "F");
        test:assertEquals(adminGenderCode.displayName, "Female");
    }

    TS? birthTime = patient.birthTime;
    if birthTime is TS {
        test:assertEquals(birthTime.value, "19750501");
    }

    CE? maritalStatus = patient.maritalStatusCode;
    if maritalStatus is CE {
        test:assertEquals(maritalStatus.code, "M");
        test:assertEquals(maritalStatus.displayName, "Married");
    }

    CE? religiousAffiliation = patient.religiousAffiliationCode;
    if religiousAffiliation is CE {
        test:assertEquals(religiousAffiliation.code, "1013");
    }

    CE[]? raceCode = patient.raceCode;

    CE[]? ethnicGroup = patient.ethnicGroupCode;

    // Assert birthplace
    Birthplace? birthplace = patient.birthplace;
    if birthplace is Birthplace {
        AD? addr = birthplace.place.addr;
        if addr is AD {
            string[]? items = addr.item;
            if items is string[] {
                test:assertEquals(items[0], "4444 Home Street");
                test:assertEquals(items[1], "Beaverton");
                test:assertEquals(items[2], "MD");
                test:assertEquals(items[3], "21014");
                test:assertEquals(items[4], "US");
            }
        }
    }

    // Assert language communication
    LanguageCommunication[]? langComm = patient.languageCommunication;
    if langComm is LanguageCommunication[] && langComm.length() > 0 {
        CS? langCode = langComm[0].languageCode;
        if langCode is CS {
            test:assertEquals(langCode.code, "en");
        }

        CE? modeCode = langComm[0].modeCode;
        if modeCode is CE {
            test:assertEquals(modeCode.code, "ESP");
        }

        CE? proficiencyCode = langComm[0].proficiencyLevelCode;
        if proficiencyCode is CE {
            test:assertEquals(proficiencyCode.code, "G");
        }

        BL? preferenceInd = langComm[0].preferenceInd;
        if preferenceInd is BL {
            test:assertEquals(preferenceInd.value, true);
        }
    }
}
